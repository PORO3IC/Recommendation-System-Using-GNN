...
class NeighborAwareConv(nn.Module):
    ...
    def forward(self, G, feat_dict):
        ...
        for srctype, etype, dsttype in G.canonical_etypes:
            ...
            G.apply_edges(fn.copy_u('rating', 'rating'), etype=etype)
...
class GAT(nn.Module):
    def __init__(self, n_users, n_items, emb_dim, 
                 n_layers, batch_size, decay, mlp_hidden_dim=128, *args, **kwargs):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(emb_dim * 2, mlp_hidden_dim),
            nn.ReLU(),
            nn.Linear(mlp_hidden_dim, mlp_hidden_dim),
            nn.ReLU(),
            nn.Linear(mlp_hidden_dim, 1)  # Output a single value
        )
    ...
    def _init_weight(self):
        U_user, _, _ = np.linalg.svd(user_latent_matrix, full_matrices=False)
        U_item, _, _ = np.linalg.svd(item_latent_matrix, full_matrices=False)
        self.user_embedding.weight.data = torch.tensor(U_user[:, :self.user_embedding.weight.size(1)], dtype=torch.float32)
        self.item_embedding.weight.data = torch.tensor(U_item[:, :self.item_embedding.weight.size(1)], dtype=torch.float32)
    ...
    def bpr_loss(self,users ,pred_scores, actual_scores, decay):

        regularizer = 1./2*(users.norm(2).pow(2) + decay.norm(2).pow(2) + decay.norm(2).pow(2))
        regularizer = regularizer / self.batch_size

        mf_loss = torch.sub((pred_scores - actual_scores))
